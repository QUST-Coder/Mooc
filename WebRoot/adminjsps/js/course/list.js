			...
			success:function(courses){
				if(!courses){
					...
					for(var i=0;i<courses.length;i++){
						...
						"<a href='/mooc2/admin/AdminCourseServlet?method=load&cou_id="+courses[i].cou_id+"' target='body'>" +
								"<img src='/mooc2/"+courses[i].cou_simg +"'/></a>"+
						"<p class='cname'>" +
						"<a href='/mooc2/admin/AdminCourseServlet?method=load&cou_id="+courses[i].cou_id+"' target='body'>" +
		...
		$.ajax({
			...
			success:function(courses){
				if(!courses){
					...
					for(var i=0;i<courses.length;i++){
						html="<div class='course'>" +
						"<a href='#'><img src='/mooc2/"+courses[i].cou_simg +"'/></a>"+
		...
		$.ajax({
			...
			success:function(courses){
				if(!courses){
					...
					for(var i=0;i<courses.length;i++){
						html="<div class='course'>" +
						"<a href='/mooc2/jsps/course/cou_desc.jsp?cou_id="+courses[i].cou_id+"' target='_blank'>" +
								"<img src='/mooc2/"+courses[i].cou_simg +"'/></a>"+
						"<p class='cname'>" +
						"<a href='/mooc2/jsps/course/cou_desc.jsp?cou_id="+courses[i].cou_id+"' target='_blank'>"+ 
						courses[i].cou_name+"</a></p>"+
